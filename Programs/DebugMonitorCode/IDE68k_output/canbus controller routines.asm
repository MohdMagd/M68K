...
; Can0_ClockDivideReg = CANMode_Bit | CBP_Bit | DivBy1 | ClkOff_Bit;
       move.b    #207,5242942
...
; Can0_OutControlReg = Tx1PshPull | Tx0PshPull | NormalMode;
       move.b    #218,5242896
...
; }
; // Configure Clock Divider Register
; Can1_ClockDivideReg = CANMode_Bit | CBP_Bit | DivBy1 | ClkOff_Bit;
       move.b    #207,5243454
; // Disable Can Interrupts
...
; Can1_BusTiming1Reg = 0x7f; // TSEG2 | TSEG1;
       move.b    #127,5243406
; // Configure CAN outputs: float on TX1, Push/Pull on TX0, normal output mode
; Can1_OutControlReg = Tx1PshPull | Tx0PshPull | NormalMode;
       move.b    #218,5243408
; /* leave the reset mode/request i.e. switch to operating mode, 
; the interrupts of the S87C654 are enabled but not the CAN interrupts of the SJA1000,
; which can be done separately for the different tasks in a system */
...
; Can0_TxBuffer3 = 0x51; /* data1 = 51 */
       move.b    #81,5242918
; Can0_TxBuffer4 = 0x52; /* data2 = 52 */
       move.b    #82,5242920
; Can0_TxBuffer5 = 0x53; /* data3 = 53 */
       move.b    #83,5242922
; Can0_TxBuffer6 = 0x54; /* data4 = 54 */
       move.b    #84,5242924
; Can0_TxBuffer7 = 0x55; /* data5 = 55 */
       move.b    #85,5242926
; Can0_TxBuffer8 = 0x56; /* data6 = 56 */
       move.b    #86,5242928
; Can0_TxBuffer9 = 0x57; /* data7 = 57 */
       move.b    #87,5242930
; Can0_TxBuffer10 = 0x58; /* data8 = 58 */
       move.b    #88,5242932
...
; printf("Can1 StatusReg = 0x%X\r\n", Can1_StatusReg);
       ...
       jsr       (A2)
       addq.w    #8,A7
; printf("Can0 Control Register = 0x%X\r\n", Can0_ModeControlReg);
       move.b    5242880,D1
       and.l     #255,D1
       move.l    D1,-(A7)
       pea       @canbus~1_3.L
       jsr       (A2)
       addq.w    #8,A7
; printf("Can1 Control Register = 0x%X\r\n", Can1_ModeControlReg);
       move.b    5243392,D1
       and.l     #255,D1
       move.l    D1,-(A7)
       pea       @canbus~1_4.L
...
CanBus0_Transmit_6:
; };
; printf("Can0 Control Register = 0x%X --Post TR--\r\n", Can0_ModeControlReg);
       move.b    5242880,D1
       and.l     #255,D1
       move.l    D1,-(A7)
       pea       @canbus~1_6.L
       jsr       (A2)
       addq.w    #8,A7
; printf("Can1 Control Register = 0x%X --Post TR--\r\n", Can1_ModeControlReg);
       move.b    5243392,D1
       and.l     #255,D1
       move.l    D1,-(A7)
       pea       @canbus~1_7.L
...
; // See section 4.2.4 in the application note for details (PELICAN MODE)
; char recievedByte;
; printf("Can0 Control Register = 0x%X -- RX --\r\n", Can0_ModeControlReg);
       move.b    5242880,D1
       and.l     #255,D1
       move.l    D1,-(A7)
       pea       @canbus~1_12.L
       jsr       (A2)
       addq.w    #8,A7
; printf("Can1 Control Register = 0x%X --RX--\r\n", Can1_ModeControlReg);
       move.b    5243392,D1
       and.l     #255,D1
       move.l    D1,-(A7)
       pea       @canbus~1_13.L
...
@canbus~1_2:
       ...
       dc.b      103,32,61,32,48,120,37,88,13,10,0
@canbus~1_3:
       dc.b      67,97,110,48,32,67,111,110,116,114,111,108,32
       dc.b      82,101,103,105,115,116,101,114,32,61,32,48,120
       dc.b      37,88,13,10,0
@canbus~1_4:
       dc.b      67,97,110,49,32,67,111,110,116,114,111,108,32
       dc.b      82,101,103,105,115,116,101,114,32,61,32,48,120
       dc.b      37,88,13,10,0
@canbus~1_5:
       dc.b      69,114,114,111,114,32,67,111,100,101,32,61,32
       dc.b      48,120,37,88,32,13,10,0
@canbus~1_6:
       dc.b      67,97,110,48,32,67,111,110,116,114,111,108,32
       dc.b      82,101,103,105,115,116,101,114,32,61,32,48,120
       dc.b      37,88,32,45,45,80,111,115,116,32,84,82,45,45
       dc.b      13,10,0
@canbus~1_7:
       dc.b      67,97,110,49,32,67,111,110,116,114,111,108,32
       dc.b      82,101,103,105,115,116,101,114,32,61,32,48,120
...
@canbus~1_9:
       ...
       dc.b      103,32,61,32,48,120,37,88,13,10,0
@canbus~1_10:
       dc.b      82,88,66,117,102,102,101,114,32,61,32,48,120
       dc.b      37,88,13,10,0
@canbus~1_11:
...
@canbus~1_12:
       dc.b      67,97,110,48,32,67,111,110,116,114,111,108,32
       dc.b      82,101,103,105,115,116,101,114,32,61,32,48,120
       dc.b      37,88,32,45,45,32,82,88,32,45,45,13,10,0
@canbus~1_13:
       dc.b      67,97,110,49,32,67,111,110,116,114,111,108,32
       dc.b      82,101,103,105,115,116,101,114,32,61,32,48,120
       dc.b      37,88,32,45,45,82,88,45,45,13,10,0
@canbus~1_14:
       dc.b      13,10,32,82,101,99,105,101,118,101,100,32,66
...
@canbus~1_16:
       dc.b      73,32,97,109,32,104,101,114,101,32,51,32,13
       dc.b      10,0
@canbus~1_17:
       dc.b      73,32,97,109,32,104,101,114,101,32,52,32,13
       dc.b      10,0
@canbus~1_18:
       dc.b      13,10,0
       xref      LMUL
